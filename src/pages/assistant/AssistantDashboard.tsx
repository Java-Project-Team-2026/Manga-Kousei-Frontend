...
const kanbanColumns: KanbanColumn[] = [
    {
        ...
        count: 3,
        cards: [
            { tag: "KIỂM SÍ CUỐI CÙNG", chapter: "Chương 42", title: "Background Page 5", action: "Tài nguyên" },
        ],
    },
    {
        id: "doing",
        ...
        count: 1,
        cards: [
            { tag: "KIỂM SÍ CUỐI CÙNG", chapter: "Chương 42", title: "Tones Page 12", deadline: "Hôm nay 18:00", warning: true, action: "Nộp kết quả" },
        ],
    },
    {
        id: "review",
        ...
        count: 2,
        cards: [
            { tag: "KIỂM SÍ CUỐI CÙNG", chapter: "Chương 41", title: "Inking Page 18", submittedAt: "Đã nộp lúc 09:30", urgent: true },
        ],
    },
    {
        id: "done",
        ...
        count: 14,
        cards: [
            { tag: "KIỂM SÍ CUỐI CÙNG", chapter: "Chương 41", title: "Tones Page 10", done: true },
        ],
    },
];
...
const renderNotifIcon = (type: Notification["icon"]) => {
    switch (type) {
        case "edit": return <SquarePen size={15} className={styles.notifIconEdit} />;
        case "file": return <FileText size={15} className={styles.notifIconFile} />;
        default: return null;
    }
};
...
const renderResourceIcon = (type: Resource["icon"]) => {
    switch (type) {
        case "user": return <UserRoundSearch size={22} />;
        case "layers": return <Boxes size={22} />;
        case "brush": return <Pencil size={22} />;
        default: return null;
    }
};
...
const KanbanCardComponent: React.FC<{ card: KanbanCard; colId: string }> = ({ card, colId }) => (
    <div className={`${styles.kanbanCard} ${card.action === "Nộp kết quả" ? styles.kanbanCardActiveStyle : ""} ${card.done ? styles.kanbanCardDone : ""}`}>
        <div className={styles.kanbanCardTop}>
            <span className={styles.kanbanTag}>{card.tag}</span>
            {card.warning && <AlertTriangle size={15} strokeWidth={2.5} className={styles.kanbanWarning} />}
            {card.urgent && <Hourglass size={15} strokeWidth={2.5} className={styles.kanbanUrgentIcon} />}
            {colId === "todo" && <MoreHorizontal size={16} className={styles.kanbanMenu} />}
        </div>

        {card.done ? (
            <div className={styles.titleWrapper}>
                <div className={styles.kanbanDoneCheckRow}>
                    {card.done && <CheckCircle2 size={16} strokeWidth={2.5} className={styles.innerDoneIcon} />}
                    <div className={`${styles.kanbanTitle} ${styles.kanbanTitleDone}`}>{card.title}</div>
                </div>
        ...
        {card.action && (
            <button className={`${styles.kanbanAction} ${colId === "doing" ? styles.kanbanActionPrimary : styles.kanbanActionSecondary}`}>
...
const KanbanBoard: React.FC = () => (
    <section className={styles.kanbanSection}>
        ...
        <div className={styles.kanbanWrapperContainer}>
            {/* Vòng tròn xanh mờ trang trí phía dưới cột Đang làm */}
            <div className={styles.circleBgDecoration} />

            <div className={styles.kanbanBoard}>
                {kanbanColumns.map((col) => (
                    <div key={col.id} className={`${styles.kanbanCol} ${styles[`kanbanCol_${col.id}`]}`}>
                        <div className={styles.kanbanColHeader}>
                            <span className={`${styles.kanbanColLabel} ${styles[`kanbanColLabel_${col.id}`]}`}>
                                {col.label}
                            </span>
                            <span className={`${styles.kanbanColCount} ${styles[`kanbanColCount_${col.id}`]}`}>
                                {col.count}
                            </span>
                        </div>
        ...
        <div className={styles.resourceGrid}>
            {resources.map((r, i) => (
                <button key={i} className={styles.resourceItem}>
                    <div className={styles.resourceItemIcon}>{renderResourceIcon(r.icon)}</div>
    ...
    return (
        <div className={styles.page}>
            <div className={styles.pageHeader}>
                ...
                <p className={styles.pageSubtitle}>Chào buổi sáng, đây là tóm tắt công việc của bạn hôm nay.</p>
            </div>

...
function KanbanCard({
  ...
  const isOverdue =
    t.taskStatus !== "done" && new Date(t.deadline) < new Date();
  ...
  return (
    <div className="at-kcard" onClick={() => onOpen(t.taskId)}>
      <div className="at-kcard__type">
        <span className={`at-status-pill ${meta.cls}`}>
      ...
      <div className="at-kcard__title">
        ...
        {t.chapterNumber != null && (
          <span>
            {" "}
            · Ch.{t.chapterNumber} · Trang {t.pageNumber}
          </span>
        )}
      </div>
      ...
      <div className="at-kcard__footer">
        <span
          className={`at-task__deadline ${isOverdue ? "at-task__deadline--overdue" : ""}`}
        >
          <Clock size={11} /> {formatDate(t.deadline)}
          {isOverdue && " · Trễ hạn!"}
        </span>
        {t.submissionCount > 0 && (
          <span className="at-task__subs">
            <FileImage size={11} /> {t.submissionCount}
          </span>
        )}
      </div>
      <div className="at-kcard__mangaka">
        {t.assignedByAvatarUrl ? (
          ...
          <span
            className="at-mini-avatar at-mini-avatar--init"
            style={{ background: getAvatarColor(t.assignedByName) }}
          >
...
export default function AssistantTasks() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setLoadingSubs(true);
      fetchTaskSubmissions(expandedId)
        .then(setSubmissions)
        .catch(console.error)
  ...
  const handleStartWork = async (taskId: number) => {
    ...
    try {
      ...
      setTasks((prev) =>
        prev.map((t) =>
          t.taskId === taskId ? { ...t, taskStatus: "doing" } : t,
        ),
      );
    } catch (err) {
      console.error(err);
  ...
  const renderPanel = (t: AssistantTaskRes) => (
    <div className="at-task__panel">
      <div className="at-task__actions">
        {t.taskStatus === "todo" && (
          <button
            className="at-btn at-btn--start"
            onClick={() => handleStartWork(t.taskId)}
            disabled={updatingStatus === t.taskId}
          >
            <Play size={13} />{" "}
            {updatingStatus === t.taskId ? "Đang cập nhật..." : "Bắt đầu làm"}
          </button>
        )}
        {(t.taskStatus === "doing" || t.taskStatus === "review") && (
          <button
            className="at-btn at-btn--submit"
            onClick={() => {
        ...
        <div className="at-submit-form">
            ...
            >
              {submitPreview ? (
                <img
                  src={submitPreview}
                  alt="Preview"
                  className="at-upload-zone__preview"
                />
              ) : (
                <>
                  <Upload size={24} strokeWidth={1.25} />
                  <span>Click để chọn file</span>
                  <span className="at-upload-zone__hint">JPG, PNG, WebP</span>
                </>
              )}
            </div>
            <input
          ...
          <div className="at-submit-form__footer">
            ...
            <button
              ...
              disabled={!submitFile || uploading || submitting}
            >
              {uploading ? (
                <>
                  <Loader2 size={13} className="at-spin" /> Upload ảnh...
                </>
              ) : submitting ? (
                <>
                  <Loader2 size={13} className="at-spin" /> Đang nộp...
                </>
              ) : (
                <>
      ...
      <div className="at-subs">
        ...
        {loadingSubs ? (
          <div className="at-subs__loading">
            <Loader2 size={14} className="at-spin" /> Đang tải...
          </div>
    ...
    <div className="at-page">
      ...
      <div className="at-header">
        ...
        <div className="at-view-toggle">
          <button
            className={`at-view-btn ${view === "list" ? "at-view-btn--active" : ""}`}
          ...
          <button
            className={`at-view-btn ${view === "kanban" ? "at-view-btn--active" : ""}`}
      ...
      <div className="at-stats">
        {(["todo", "doing", "review", "done"] as const).map((s) => {
          ...
          return (
            <div
              key={s}
              className={`at-stat-card ${filter === s ? "at-stat-card--on" : ""}`}
      ...
      <div className="at-search">
        <Search size={15} />
        <input
          placeholder="Tìm theo series, loại task..."
          value={search}
      ...
      {loading ? (
        <div className="at-empty">
          <Loader2 size={24} className="at-spin" /> Đang tải...
        </div>
      ) : view === "list" ? (
          ...
          <div className="at-list">
            {filtered.map((t) => {
              ...
              return (
                <div
                  key={t.taskId}
                  className={`at-task ${isExpanded ? "at-task--open" : ""}`}
                >
                  <button
                    className="at-task__head"
                    onClick={() => setExpandedId(isExpanded ? null : t.taskId)}
                  >
                    <div className="at-task__preview">
                      <img src={t.pageFileUrl} alt="" />
                      <div
                        className="at-task__region-overlay"
                        style={{
                          left: `${t.regionX}%`,
                          top: `${t.regionY}%`,
                          width: `${t.regionWidth}%`,
                    ...
                    <div className="at-task__info">
                      ...
                      <div className="at-task__context">
                        {t.seriesTitle} · Chương {t.chapterNumber} · Trang{" "}
                        {t.pageNumber}
                      </div>
                      {t.description && (
                        <p className="at-task__desc">{t.description}</p>
                      )}
                      <div className="at-task__meta">
                        <span className="at-task__mangaka">
                          {t.assignedByAvatarUrl ? (
                            <img
                              src={t.assignedByAvatarUrl}
                              alt=""
                              className="at-mini-avatar"
                            />
                          ) : t.assignedByName ? (
                            <span
                              className="at-mini-avatar at-mini-avatar--init"
                              style={{
                                background: getAvatarColor(t.assignedByName),
                              }}
                            >
                              {getInitials(t.assignedByName)}
                            </span>
                        ...
                        {t.submissionCount > 0 && (
                          <span className="at-task__subs">
                            <FileImage size={11} /> {t.submissionCount} lần nộp
                          </span>
                        )}
                      </div>
                    </div>
            ...
            <div className="at-modal__head">
              <div>
                <div className="at-modal__title">
                  ...
                  {expandedTask.chapterNumber != null &&
                    ` · Ch.${expandedTask.chapterNumber} · Trang ${expandedTask.pageNumber}`}
                </div>
                <div className="at-modal__sub">
                  {expandedTask.seriesTitle} · {expandedTask.assignedByName}
                </div>
              </div>
              <button
                onClick={() => {
